...
`두 실행 비교`에서 첫 번째 실행을 선택한 뒤 다른 행을 누르면 두 실행을 비교합니다. 비교 화면은 결과 화면과 같은 막대와 눈금으로 그리며, 먼저 고른 실행이 이 비교의 기준(Baseline)으로 눈금이 되고, 나중에 고른 실행이 비교 대상(Compare)으로 막대가 됩니다. 먼저 고른 실행이 지정된 baseline이 아니어도 그 실행을 기준으로 회귀를 판정하며, 지정된 baseline은 바뀌지 않습니다. 비교 화면의 `두 run 바꾸기`로 두 run의 역할을 바꿀 수 있습니다.
