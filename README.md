...
`두 실행 비교`에서 첫 번째 실행을 선택한 뒤 다른 행을 누르면 두 실행을 비교합니다. 비교 결과는 결과 화면의 `비교`와 같은 변화율 막대 그래프로 표시합니다. 선택한 기준이 현재 baseline일 때만 회귀 판정(빨강·파랑 막대)을 표시하고, 그 외에는 회색 막대로 변화량만 표시합니다. 비교 화면에서 기준과 현재 실행을 바꿀 수 있습니다.
