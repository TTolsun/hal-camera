...
import { REPO_ROOT, normalizeText } from './lib.mjs';
...
export function manuscriptFiles(bindings, key) {
  ...
  if (!files.length) throw new Error(`${key.page}/${key.block.id}: 코드 근거가 없습니다. node tools/docgen/brief.mjs ${key.page} ${key.block.id}로 첫 원고를 작성하고 sources를 지정하세요.`);
  for (const file of files) {
    if (!fs.existsSync(safePath(REPO_ROOT, file))) throw new Error(`${key.page}/${key.block.id}: 인용 파일이 없습니다: ${file}`);
  }
  return files;
}
...
export function manuscriptPlan(key, brief, files) {
  ...
  const sources = files.map(file => ({ file, text: normalizeText(fs.readFileSync(safePath(REPO_ROOT, file), 'utf8')) }));
