...
코드와 전체 집필 프롬프트가 기본 60,000자 안에 들어오면 원본 코드를 그대로 제공합니다. 한도를 넘으면 선택한 코드를 여러 호출로 나누어 근거 요약을 만들고, 전체 `brief.mjs` 출력과 요약으로 원고를 작성합니다. 모든 원본 문자와 파일 경로를 분할 입력에 포함하며, 큰 파일의 각 부분에는 문자 범위를 표시합니다. 입력을 잘라 버리거나 기본 한도를 올리지 않습니다. `.omm` 필드, 집필 규칙, 기존 원고, 사실과 사람의 검증 기록은 기존 brief 그대로 유지합니다.
