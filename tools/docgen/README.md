...
각 요약과 최종 원고 입력에 한도를 적용합니다. 빈 요약·요약 길이 초과·원고 계약 위반은 전체 트랜잭션 실패이며, 요약은 검토 승인이나 영구 캐시로 저장하지 않습니다. 근거 요약은 모델의 중간 해석이므로 생성 PR에서 원본 코드를 대조해야 합니다. `--write-only --force --dry-run`으로 모델 호출 없이 분할 개수와 입력 크기를 확인할 수 있습니다.
