...
const config = JSON.parse(fs.readFileSync(path.join(root, 'tools/docgen/project.json'), 'utf8'));
if (command === 'design' && config.design?.preset === 'custom') {
  const {generateCustomDesign} = await import('./design.mjs');
  generateCustomDesign(root, config.design, args);
  process.exit(0);
}
