#!/usr/bin/env node
// 정적 사이트 빌더. guide/ 의 Markdown 원고를 docs/ 의 HTML 로 만듭니다.
//
// Jekyll 을 대체합니다. GitHub Pages 는 "Deploy from a branch → main → /docs" 로
// 설정하고, docs/.nojekyll 로 GitHub 측 빌드를 끕니다. 사내 GitHub 미러도 같은
// 설정만으로 같은 산출물을 제공합니다. 빌드에 필요한 것은 Node 24 와
// `npm ci --prefix tools/docgen --ignore-scripts` (Markdown 파서 marked) 뿐입니다.
//
// 명령
//   node tools/docgen/site.mjs build           guide/ → docs/ 에 씁니다. 매니페스트에 있는 이전 산출물만 정리합니다.
//   node tools/docgen/site.mjs check           디스크의 docs/ 가 지금 빌드한 결과와 같은지 검사합니다. 다르면 종료 코드 1.
//   node tools/docgen/site.mjs serve [--port N] [--base /hal-camera/]
//                                              docs/ 를 하위 경로 아래에서 미리 봅니다. 배포 URL 의 하위 경로 차이를 확인합니다.
//
// 규칙
...
//   - docs/ 에는 설계·검증 문서도 있으므로 매니페스트(docs/.site-manifest.json)에 기록된 파일만 지우거나 덮어씁니다.
...
export async function renderSite(site) {
  ...
  for (const page of pages) {
    ...
    const vars = {
      ...
      page_label: meta.home ? "" : '<p class="eyebrow page-label">HAL CAMERA / Design Documentation</p>',
      content: html,
    };
...
async function loadMarked() {
  try {
    ...
    throw new Error("marked 를 찾을 수 없습니다. 먼저 `npm ci --prefix tools/docgen --ignore-scripts` 를 실행하세요.");
  }
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(import.meta.filename)) {
  ...
  try {
    ...
    if (command === "build") {
      ...
      if (problems.length) {
        console.error(`사이트 산출물 불일치 ${problems.length}건:\n  ${problems.join("\n  ")}\n'node tools/docgen/site.mjs build' 를 실행해 반영한 뒤 커밋하세요.`);
        process.exit(1);
      }
