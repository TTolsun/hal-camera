...
2026-09-12 · HAL CAM 0.5.1-dev / `feature/bm-m5-score`. M5a 내부 점수 초안을 구현했으며 민감도 검증은 진행 중입니다.
...
검증: `assembleDebug assembleRelease testDebugUnitTest lintDebug` 통과, 단위 테스트 261개 성공. M5a는 초안 구현 상태이며 발열·카메라 점유 경쟁 측정, 민감도 재검증과 코드 리뷰가 남았습니다. M5b에 필요한 다기기 분포도 확보하지 못했으므로 #9와 #4는 열린 상태로 유지합니다.
...
3. 내부 점수 초안의 계산 규칙과 실기기 결과를 검토한 뒤 M5a 완료 여부를 결정합니다.
