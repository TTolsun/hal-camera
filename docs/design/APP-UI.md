...
**앱을 켜면 바로 Live 프리뷰가 나옵니다.** Exynos Camera HAL 개발자가 여러 시험 기기에서 같은 작업을 반복하는 흐름을 우선합니다. Live는 프리뷰와 촬영, 현재 세션의 진단을 담당하며 선택적인 작업실은 도구 메뉴에서 엽니다. 앱과 문서 사이트는 HAL-CAMERA-Editorial을 공통 디자인 기준으로 사용합니다.

색상·글꼴·선·모서리의 공통 기준은 [DESIGN.md](DESIGN.md)에서 관리합니다. 이 문서는 그 기준을 적용한 Android 앱의 배치와 조작을 설명합니다.

## 작업실과 기기 식별

Live의 `도구 → 작업실로 이동`에서 기기 모델, SoC, Android 버전, API 수준과 OS 빌드를 확인합니다. 작업실 자체는 카메라를 열거나 촬영 권한을 요구하지 않습니다. SoC는 기기가 보고한 값을 사용하며 Exynos로 가정하지 않습니다. API 31 미만에서는 `Build.HARDWARE`를 표시합니다.

1. `기기 이름·빌드 정보`에서 같은 모델의 시험 기기를 구별할 별칭을 저장합니다. 별칭은 앱 로컬 설정이며 측정 빌드 라벨이나 보고서의 기기 계약을 바꾸지 않습니다.
2. 촬영과 실시간 진단은 `Live · 카메라 열기`로 시작합니다.
3. `Probe · 사양 확인`, `CTS · 동작 검증`, `Benchmark · 성능 측정`에서 검사 목적을 선택합니다.
4. 저장된 실행의 비교·내보내기는 `실행 기록`, 촬영물 확인은 `갤러리`로 이동합니다.

공통 도구 제목 아래에는 별칭과 모델을 표시합니다. 기기 정보 대화상자는 OS 빌드와 fingerprint 복사를 제공하며 하드웨어 serial을 읽지 않습니다. 작업실은 실제 창 너비를 기준으로 본문 폭을 최대 840dp로 제한하고 긴 내용은 세로로 스크롤합니다. 시스템 표시줄과 디스플레이 컷아웃은 인셋으로 피합니다.
...
| 축약 | `Camera · 0`, `Camera · 2` | 칸이 좁은 자리. CTS 진행 줄과 보고서 줄, 실행 기록 필터 버튼입니다. |
...
| 영역 | 구성과 목적 |
| --- | --- |
| 상단 | 현재 API, `도구`, `진단`을 배경·테두리 없는 글자로 표시합니다. API 버튼을 누를 때마다 Camera2↔CameraX로 전환합니다. API 아래에는 작은 원과 `Live`를 두고, 실제 프리뷰가 동작할 때만 둘 다 빨간색으로 깜빡입니다. 프리뷰가 정지하면 회색으로 고정하며, 시스템 애니메이션이 꺼져 있으면 동작 중에도 빨간색을 고정합니다. |
| 카메라 제어 | 상단 중앙에 글씨 없이 아래 화살표만 두고, 누르면 플래시·AF·AE·EV 원형 버튼 줄을 펼칩니다. 쓰지 않는 제어가 프리뷰를 가리지 않도록 평소에는 접어 둡니다. 펼친 동안에는 위 화살표로 바뀝니다. 켜 둔 제어는 펼친 버튼과 화살표의 접근성 이름·길게 누르기 설명에서 확인합니다. 버튼의 켜짐은 줌 레일과 같은 흰 원으로, 잠금은 글자 아래 자물쇠로 표시합니다. 플래시는 같은 자리에서 Off·Auto·On·Torch 선택지로 바뀌고, EV는 버튼 줄 아래 한 줄에 눈금 다이얼과 `− / + / 0` 버튼을 엽니다. 한 단계의 크기와 범위는 기기의 지원값을 따릅니다. 버튼 아래에 상시 설명을 붙이지 않으며, 접근성 이름과 길게 누르기 설명으로 동작을 확인합니다. 지원하지 않는 제어는 흐리게 두고 누르면 이유를 알립니다. |
| 프리뷰 | 검은 배경 위에 카메라 영상을 최대한 넓게 표시합니다. 조작에 필요한 글자와 버튼의 대비는 유지하며, 영상을 가리는 타일과 상시 그래프는 줄입니다. |
| 하단 측정 영역 | 별도 타일 없이 최대 2줄로 얇게 표시합니다. 첫 줄은 FPS·ISO·노출, 둘째 줄은 AE·AF 상태입니다. 줌 레일과 제어 버튼이 이미 보여 주는 값은 반복하지 않으며, 플래시 상태와 요청과 다른 EV·줌, 물리 렌즈는 둘째 줄 폭이 허락할 때만 이 순서로 붙습니다. 프레임 간격 그래프와 Partial·Buffer 관측값은 `진단` 패널에서 제공합니다. |
| 하단 촬영 영역 | 지원하는 줌 배율, 셔터 행, 사진·동영상 모드의 순서로 배치합니다. 셔터 행은 왼쪽 최근 촬영물 썸네일, 중앙 셔터, 오른쪽 카메라 선택으로 구성합니다. |
| 고정 작업 행 | `이벤트 저장 · ZIP`을 항상 표시합니다. 버튼과 작업 행 자체의 배경은 투명합니다. 촬영 조작부와 Mark 행이 하나의 은은한 그라데이션을 공유해 색 경계 없이 이어지며 Mark 글씨에는 그림자를 적용합니다. 진단 패널을 스크롤해도 위치를 유지합니다. |
...
1. 상단 막대: API 버튼과 그 아래 Live 표시등, 중앙의 촬영 제어 화살표, `도구`, `진단`
2. 프리뷰
...
Benchmark는 메인 프리뷰에 별도 버튼을 두지 않고 상단 `도구` 메뉴에서 엽니다. 메뉴는 `ui/SelectionPopup.kt`의 `showActionPopup` 목록으로 `Probe · 사양 확인`·`CTS · 동작 검증`·`Benchmark · 성능 측정`·`실행 기록 · 비교`·`작업실로 이동`을 표시합니다. 값을 고르는 목록이 아니라 이동할 곳의 목록이므로 라디오 버튼을 붙이지 않습니다. 각 기능은 독립 화면(`CameraProbeActivity`, `CtsEntryActivity`, `BenchmarkActivity`)을 엽니다. Probe와 Benchmark는 현재 카메라 ID를 초기값으로 받으며 자체 카메라 선택을 제공합니다. CTS는 커스텀 케이스와 AOSP 원문 중 경로를 고른 뒤 체크리스트에서 실행할 항목을 선택합니다.
...
작업실 진입도 Live 세션의 종료를 기다립니다. 실행 기록은 카메라를 열지 않으므로 바로 이동합니다. `CTS`와 `Benchmark`는 자기 카메라를 열기 때문에 Live 세션을 닫고 `close(done)` 콜백을 받은 뒤에 화면을 엽니다. 이 대기 때문에 전환이 즉시 일어나지 않을 수 있으며, 그동안 메뉴를 다시 누를 수 없게 합니다. 녹화 중과 저장 중에는 `도구` 메뉴 전체를 비활성화합니다. `Probe`는 카메라를 열지 않으므로 닫기 완료를 기다리지 않고 바로 엽니다. 이때 Live 카메라는 화면이 가려질 때 평소처럼 닫히고 돌아오면 다시 열립니다.
...
모드나 카메라를 선택하는 동작은 설정만 바꿉니다. 촬영과 녹화는 중앙 셔터를 눌러 실행합니다. 녹화 중에는 모드 위치에 경과 시간을 표시합니다. 엔진·카메라·모드 변경과 일시정지·갤러리·`도구` 메뉴는 비활성화합니다. 줌과 카메라 제어(토치·잠금·EV)는 녹화를 멈추지 않고 바꿀 수 있습니다. 정지 셔터를 누르면 `저장 중…`을 표시하고, 녹화 종료 처리 동안 셔터를 비활성화해 중복 정지를 막습니다. 앨범 저장 완료는 별도 알림으로 표시합니다.
