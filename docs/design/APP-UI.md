...
| 영역 | 구성과 목적 |
| --- | --- |
| 상단 | 현재 API, `도구`, `진단`을 배경·테두리 없는 글자로 표시합니다. API 버튼을 누를 때마다 Camera2↔CameraX로 전환하며, 메뉴 사이에 `Camera2 · LIVE`처럼 엔진과 상태를 같은 줄로 표시합니다. |
...
| 진단 패널 | 열려 있는 세션에서만 의미가 있는 항목을 제공합니다. 카메라 ID 선택, 일시정지·재개, `MARK · ZIP 저장`, LIVE READOUT, 프레임 간격·3A·콜백 타임라인 그래프, incident ZIP 공유·기록 목록, 권한/재시도, 측정 안내, ADB CLI 허용입니다. |
...
LIVE 세션과 무관하게 동작하는 도구는 진단 패널에 두지 않고 상단 `도구` 메뉴로 모읍니다. 메뉴는 카메라 선택과 같은 `SelectionPopup` 목록이며 `Benchmark`, `PROBE`, `CTS` 세 항목을 표시합니다. 각 항목은 독립 화면(`BenchmarkActivity`, `CameraProbeActivity`, `CtsCaseActivity`)을 열고, LIVE의 현재 엔진과 카메라 ID를 초기값으로만 넘깁니다. 세 화면 모두 자체 카메라 선택 목록을 가지므로 LIVE로 돌아와 카메라를 바꾼 뒤 다시 들어갈 필요가 없습니다.
...
LIVE에서는 실제로 관측한 수치와 단위를 표시합니다. MARK는 직전 10초와 이후 5초를 incident ZIP으로 저장하는 작업이며, 사진 셔터와 구별합니다. 열려 있는 세션의 자세한 그래프와 incident 내보내기는 `진단` 패널에서, 세션과 무관한 벤치마크·사양 조회·CTS 실행은 `도구` 메뉴의 독립 화면에서 제공합니다. 어떤 항목을 어디에 둘지는 "현재 세션이 열려 있어야 의미가 있는가"로 판단합니다.

버튼의 현재 값, 요청한 줌, capture result에서 읽은 적용 줌의 의미를 유지합니다. 시각 구성을 바꾸더라도 측정 계약, 스트림 조건, 결과 비교 규칙은 변경하지 않습니다.

## 구현과 확인 범위

LIVE 배치는 `MainActivity.kt`, 셔터·줌·선택 목록은 `ui/ShutterButton.kt`·`ui/ExpandingZoomControl.kt`·`ui/SelectionPopup.kt`에서 관리합니다. 최근 촬영물은 `camera/RecentMediaThumbnail.kt`와 `ui/RecentMediaButton.kt`에서 조회·표시합니다. 공통 아이콘 동작은 `ui/IconButton.kt`, 갤러리는 `GalleryActivity.kt`와 `ui/GalleryImageView.kt`에서 관리합니다. 도구 메뉴의 독립 화면은 `benchmark/BenchmarkActivity.kt`, `CameraProbeActivity.kt`, `cts/CtsCaseActivity.kt`입니다. UI 변경을 검토할 때에는 프리뷰를 가리는 영역, 가독성, 터치 영역, 실제 상태와 아이콘의 일치, 접근성 설명을 확인합니다. 기기 관찰과 테스트 결과는 해당 변경의 검토 기록에 별도로 남깁니다.
