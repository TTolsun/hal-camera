...
괄호가 같은 항목이 둘 이상 나올 수 있으며, 이것은 표기의 결함이 아닙니다. Galaxy S25+에서 `Camera · 1 (Front)`와 `Camera · 3 (Front)`가 그런 경우인데, 두 항목은 초점 거리가 3.3mm로 동일한 전면 렌즈 하나를 센서 크롭만 달리해 노출한 것입니다(35mm 환산 약 26mm와 30mm). 렌즈가 실제로 같으므로 렌즈 이름도 같게 두고, 두 항목은 키로 구분합니다. 전면에 렌즈 추론을 적용하더라도 두 값이 모두 `Wide` 구간(20–35mm)에 들어가므로 구분되지 않습니다. 화각까지 구분해야 할 이유가 생기면 그때 괄호에 환산 초점 거리를 넣는 방안을 검토합니다.
...
Benchmark 결과는 판정 한 줄을 먼저 표시하고, 그 아래에 측정한 모든 항목을 카테고리별 막대로 그립니다. 값과 기준을 나란히 읽는 것이 이 화면의 목적이므로 숫자 표를 접어 두지 않습니다. 실행 정보는 `실행 정보` 접힘에 라벨과 값 한 쌍씩 두고(비교 화면의 같은 자리는 `Run info`입니다), validity flag는 저장된 코드가 아니라 뜻으로 적습니다. 결과를 PC로 옮기는 경로는 `Export` 하나로 유지하며 화면 텍스트를 클립보드로 복사하는 버튼은 두지 않습니다. 판정 문구와 화면 요소의 자세한 내용은 [Benchmark 가이드](../../guide/benchmark.md)에서 관리합니다.

Results의 각 행은 실행 시각과 배지, 사실 한 줄의 두 줄로 적습니다. run id와 profile, 원본 flag는 행이 아니라 행이 여는 결과 화면에 둡니다. `두 실행 비교`로 기준과 현재 실행을 고르며 선택 기준을 테두리로 표시합니다. 각 행의 `⋮` 메뉴는 baseline 지정·비교·내보내기 등을 제공하고 길게 누르기도 유지합니다. 보관 한도는 `Data limit` 슬라이더로 고르며, 눈금 간격을 일정하게 두어 같은 거리가 같은 변화를 뜻하게 합니다.
