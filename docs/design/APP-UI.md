...
실행 기록의 각 행은 실행 시각과 배지, 사실 한 줄의 두 줄로 적습니다. run id와 profile, 원본 flag는 행이 아니라 행이 여는 결과 화면에 둡니다. `두 실행 비교`로 기준과 현재 실행을 고르며 선택 기준을 테두리로 표시합니다. 각 행의 `⋮` 메뉴는 baseline 지정·비교·내보내기 등을 제공하고 길게 누르기도 유지합니다. 보관 한도는 `Data limit` 슬라이더로 고르며, 눈금 간격을 일정하게 두어 같은 거리가 같은 변화를 뜻하게 합니다.
