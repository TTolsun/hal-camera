...
괄호 안의 세 칸은 아래 순서로 채웁니다. 방향 칸은 HAL이 보고한 `LENS_FACING` 값을 먼저 따르며, 값이 없을 때 역할이 `FRONT`인 카메라만 `Front`로 적습니다. 역할이 `EXTERNAL`이어도 보고값이 없으면 방향을 적지 않습니다. 열거 과정이 `LENS_FACING`을 읽지 못한 카메라에도 `EXTERNAL`을 붙이기 때문입니다.
...
![Live 화면의 영역 배치 개념도](figures/live-layout.svg)

그림은 표의 하단 촬영 영역을 줌·셔터 행·모드의 세 줄로 나누어 번호를 붙였으며, 진단 패널은 생략했습니다. 오른쪽 아래의 셔터 세 상태는 다음 단락의 설명을 그린 것입니다. 기기 화면을 캡처한 그림이 아니므로, 배치가 바뀌면 `figures/live-layout.svg`도 함께 고칩니다.
