...
| 카메라 제어 | 상태 줄 아래 가운데에 `Flash · AF · AE · EV` 이름을 적은 알약을 두고, 누르면 플래시·AF·AE·EV 원형 버튼 줄을 펼칩니다. 쓰지 않는 제어가 프리뷰를 가리지 않도록 평소에는 접어 둡니다. 접힌 알약에는 켜 둔 제어를 `AF lock · EV +0.7`처럼 이어 적어, 숨은 설정이 사진을 바꾸지 않게 합니다. 버튼의 켜짐은 줌 레일과 같은 흰 원으로, 잠금은 글자 아래 자물쇠로 표시합니다. 플래시는 같은 자리에서 Off·Auto·On·Torch 선택지로 바뀌고, EV는 버튼 줄 아래 한 줄에 눈금 다이얼과 `− / + / 0` 버튼을 엽니다. 한 단계의 크기와 범위는 기기의 지원값을 따릅니다. 버튼 아래에 상시 설명을 붙이지 않으며, 접근성 이름과 길게 누르기 설명으로 동작을 확인합니다. 지원하지 않는 제어는 흐리게 두고 누르면 이유를 알립니다. |
