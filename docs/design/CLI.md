...
> 2026-09-20 범위 변경: 기본 CLI는 `adb`와 APK에 포함된 기기용 `halcam.sh`입니다. 프리뷰 시작·종료, 사진 캡처, 녹화 시작·종료를 지원하며 `benchmark.run`은 접수하지 않습니다. 현재 사용법과 준비 절차는 [CLI 가이드](../../guide/cli.md)를 따릅니다. 아래 내용은 최초 구현의 설계 기록이며, Python 필수 조건과 벤치마크 명령·녹화 미지원 범위는 현재 계약에 적용하지 않습니다.


작성일: 2026-09-12 · 상태: 0.6.0 릴리스 검증 · 관련 이슈: [#56](https://github.com/TTolsun/hal-camera/issues/56)

이 문서는 PC에서 ADB로 Android 앱을 제어하는 설계와 검증 기준이다. 기본 명령은 앱 0.6.0과 Python CLI 0.1.0에 구현했다. 실제 통과 범위는 [기능 검증 기록](../validation/cli-device.md)에 기록한다. 실제 파일 구조와 확정된 정책을 아래에 기록한다. 구현 순서와 검증 기준은 [구현 계획](../PLAN-CLI-v0.1.md)에 정의한다.
...
0.5.1 main(`0a1d71e`)의 카메라·갤러리 UI를 통합한 코드를 기준으로 검토했다.
...
| [CameraEngine](../../app/src/main/java/dev/halcamera/camera/CameraEngine.kt) | 기존 `capture()`를 유지하고 Camera2의 `capturePhoto()`가 타입 기반 완료 결과를 제공한다. |
...
flowchart LR
    ...
    C --> A[ADB]
    A -->|앱 열기| G[CliLaunchActivity]
    G -->|작업 상태 재확인| M[MainActivity]
    ...
    Q --> F[CommandCoordinator artifact 등록]
    P -->|읽기 전용 스트림| F
```
...
`CommandStore`는 요청·상태·결과 식별자를 앱 내부에 보관한다. artifact 등록·읽기 책임은 별도 클래스 대신 `CommandCoordinator`에 두며 CLI 요청이 생성한 결과만 노출한다. 기존 갤러리 전체나 임의의 앱 내부 파일을 조회하는 API로 확대하지 않는다.
...
앱 설정에 **ADB CLI 허용** 스위치를 추가하고 초기값은 꺼짐이다. 활성화는 사용자 0의 앱 설정에 저장하여 재실행 후에도 유지한다. 비활성화하면 신규 명령과 파일 접근을 막고 실행 중 작업은 취소 절차를 진행한다. 비활성화 상태의 shell에는 파일이나 실행 이력이 없는 `CLI_DISABLED` 진단만 반환한다. 이 설정은 ADB로 승인된 모든 PC에 적용되며 PC별 접근 권한은 ADB 인증이 담당한다.
...
명령 정규화 후 요청 ID와 정규화된 요청 내용를 원자적으로 저장하고 동작을 시작한다. 같은 ID·같은 내용은 기존 결과를 반환하며, 같은 ID·다른 내용은 `REQUEST_CONFLICT`로 거부한다. 중복 방지 보장은 기록 보존 기간 안에서만 제공한다. 기록이 사라진 요청을 클라이언트가 자동 재제출하지 않는다.
...
앱 실행 제한의 기본값은 준비 포함 preview·capture 30초, benchmark 180초이며 상한은 3,600초다. 앱은 monotonic clock을 사용한다. 앱 실행 제한, PC 대기 제한, 파일 전송 제한은 별개다. PC 대기 제한이나 연결 끊김은 앱의 실패를 의미하지 않는다. CLI는 요청 ID와 확인 방법을 반환하며 자동 재촬영하지 않는다.
...
결과는 `files/cli/requests/<uuid>.json`의 원자적 상태 기록과 등록된 artifact ID로 관리한다. 사진은 MediaStore URI, benchmark는 기존 report 파일을 참조한다. 요청 ID와 capture ID·센서 시각 또는 run ID의 매핑을 명시적으로 보관하며 최신 파일명이나 logcat 검색으로 추정하지 않는다.

초기 보존 정책은 완료 후 24시간, 완료 요청 최대 200개다. 활성 요청은 정리하지 않는다. 상한 때문에 먼저 만료될 수 있으므로 응답에 만료 시각과 보존 상한을 알린다. 정리는 CLI 메타데이터·전송용 임시 파일만 대상으로 하며 원본 사진과 기존 benchmark 이력을 삭제하지 않는다. 사용자가 원본을 삭제하면 파일 회수는 종료 코드 7로 실패한다.
...
| 항목 | 확정 정책 | 검증 범위 |
|---|---|---|
| 실행·배포 | Python 3.11+, PC + ADB, wheel·저장소 설치 | Windows 실기기, 3개 PC OS의 CI |
| 전송·접근 | shell UID 2000 + DUMP, 스위치 기본 꺼짐 | API 26·36의 일반 앱 접근 거부 |
| 시간·보존 | 30/180초 기본, 3,600초 상한, 완료 후 24시간·200개 | 시간 초과·만료·상한 테스트 |
| Android | 사용자 0, API 26 이상, 잠금 해제·전면 화면 | API 26 에뮬레이터, Galaxy S25+ API 36 |

상태 조회는 메모리 snapshot을 읽으며 파일 쓰기는 상태 전환 시에만 한다. 파일 저장소에 쓰지 못하면 해당 프로세스에서는 `STORE_FAILED`와 `durable=false`로 실패를 알리고 추가 실행을 하지 않는다. 재시작 시 마지막 영속 기록은 `interrupted`로 복구한다.

제출 스키마는 `tools/halcam/protocol-v1.schema.json`, 상태 응답 스키마는 `tools/halcam/record-v1.schema.json`, Android·Python 공통 fixture는 `tools/halcam/fixtures/protocol-v1.json`이다. 다른 OEM 전체와 Linux·macOS의 실제 ADB 장치 연결을 검증한 것으로 확대하지 않는다.
