...
카메라는 역할과 ID가 있는 목록에서 직접 선택합니다. 빌드 정보는 접힌 상태에서도 저장된 값을 요약합니다. 결과는 회귀 지표를 먼저 표시하고, 숫자 행은 화면 폭에 맞춰 줄바꿈합니다. 전체 지표는 회귀가 있으면 접고 없으면 펼칩니다. 실행 정보·점수는 펼쳐 보며 복사 버튼으로 원래 텍스트를 복사합니다. HistoryActivity에서 연 실행은 저장된 기기 정보를 표시하고 새 실행 버튼을 숨깁니다.
