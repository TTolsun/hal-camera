...
LIVE의 진단 패널은 뒤로 가기로 닫힙니다. 도구 메뉴에서 Benchmark나 CTS를 고르면 LIVE 카메라를 닫고 close(done) 뒤에 화면을 열며, PROBE는 세션을 유지한 채 엽니다. 카메라 관측과 녹화 중에는 화면을 유지하고 일시정지·비활성 상태에서는 화면 유지 플래그를 해제합니다. 녹화 세션이 닫히면 MediaRecorder를 종료·저장하고 활성 카메라는 사진용 프리뷰 세션으로 복귀합니다.
