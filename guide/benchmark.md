...
| `실행 정보` 접힘 | 기기, 카메라, 시각, OS 빌드, subject, 표본 수, 발열, 비고, 파일을 라벨과 값 한 쌍씩 표시합니다. 그 아래에 눈금이 가리키는 run(`Baseline`, `Baseline OS`, `Baseline 발열`)과 두 run의 빌드 비교를 이어 적습니다. 이전 run과 비교할 때는 라벨도 `이전 run`이 됩니다. 화면의 다른 곳과 같은 이름을 써야 같은 run임을 바로 알 수 있기 때문입니다. |
...
시간 값은 표본의 **중앙값**입니다. 행마다 `median`을 붙이지 않고, 지표 카드 맨 위의 범례 아래 줄에 `대표값: Median`으로 한 번만 적습니다. 범례의 막대 조각과 눈금은 아래 지표 막대와 같은 모양으로 그려, 막대는 `비교 대상(Compare)`, 눈금은 `기준(Baseline)`(baseline이 없으면 `이전 run`)임을 가리킵니다. `━`·`│` 같은 글자 기호는 실제 막대와 모양이 달라 무엇을 뜻하는지 연결되지 않았기 때문입니다. 지표 이름 아래에는 그 지표가 재는 Camera2 구간을 작게 적습니다(예: `Open  openCamera → onOpened`, `Partial  onCaptureStarted → onCaptureCompleted`). 정의 원문은 `docs/METRICS.md`에 있고, `First frame`처럼 구현이 표와 다른 지표는 구현 기준(`openCamera → 첫 YUV`)으로 적습니다. `Preview` 묶음은 `Frame rate`가 먼저 오고 그 간격 행이 뒤따릅니다. `Frame rate`는 `H.1`을 뒤집은 값이며, 판정 자체는 `H.1` 행이 가집니다.
...
- 기준 run에만 있는 지표도 행을 남깁니다. 이 경우 값은 `—`이고 눈금만 그리며, 막대 아래에 `Baseline에만 있음`이라고 적습니다.
- 판정하지 못한 이유(`조건 불일치`, `표본 부족`, `단위 다름` 등)를 막대 아래에 적습니다.
- 변화량 숫자의 색은 판정만 뜻합니다. 빨강은 저하, 초록은 개선, 회색은 판정하지 않은 변화입니다. 판정은 Baseline(지정된 baseline, 또는 두 실행 비교에서 먼저 고른 run)과 비교할 때만 하므로, 이전 run과 비교할 때는 빨간 막대도 나오지 않습니다.
...
1. `두 실행 비교`를 누르고, 눈금으로 둘 run과 비교할 run을 차례로 고릅니다. 먼저 고른 run은 밝은 파란 테두리로 표시하며, 취소나 뒤로 가기로 선택을 해제합니다. 안내는 `기준(Baseline)을 선택하세요.`, 그다음 `Baseline: 09-26 16:00` / `비교 대상(Compare)을 선택하세요.` 순서입니다. 먼저 고른 run은 이 비교의 Baseline이 되어 눈금으로 그려지고, 지정된 baseline이 아니어도 그 run을 기준으로 저하·개선을 판정합니다. 화면이 그 run을 Baseline이라고 부르므로 판정도 그에 맞춥니다. 목록의 지정된 baseline은 바뀌지 않습니다. 나중에 고른 run은 `비교 대상(Compare)`으로 막대가 됩니다. 화면에서는 `기준`이라는 말을 쓰지 않습니다. 결과 화면이 같은 run을 baseline이라고 부르므로, 다른 이름이 섞이면 같은 run인지 다시 확인해야 하기 때문입니다.
...
저장 직후에는 `설정`의 `최대 보관 개수`가 적용됩니다. 한도를 넘는 실행 파일을 오래된 것부터 지우며 baseline으로 지정한 실행은 지우지 않습니다. 한도는 슬라이더로 고르고 10부터 100까지 10 단위이며, 그 끝에 `무제한`이 한 칸 더 있습니다. 기본값은 `무제한`입니다. 눈금 간격을 일정하게 둔 이유는, 슬라이더가 "같은 거리는 같은 변화"를 약속하기 때문입니다.
