...
`Record` 묶음의 숫자는 단위가 서로 다릅니다. `Steady fps`는 클수록 좋은 유일한 지표이므로 값이 내려갈 때 저하로 판정하며, `Interval anomalies`는 개수입니다. `Record jitter`와 `Steady fps`는 소수 한 자리를 유지합니다. 29.8 fps는 창 하나에서 프레임을 놓쳤다는 뜻이고, jitter는 1밀리초보다 작아서 정수로 자르면 모든 값이 0으로 찍히기 때문입니다.
