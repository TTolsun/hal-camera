...
`두 실행 비교`와 `CSV 내보내기`는 결과 화면의 `비교`·`내보내기`처럼 한 줄에 나란히 둡니다. 전체 폭 버튼 두 개와 개수 줄이 화면 위쪽 절반을 차지해 첫 실행이 화면 중간에서야 보였기 때문입니다. `CSV 내보내기`는 지금 필터에 보이는 실행을 모두 내보내며, 몇 개인지는 목록 제목과 화면 낭독기 설명에 있습니다.
