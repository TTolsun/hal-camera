...
| `실행 정보` 접힘 | 기기, 카메라, 시각, OS 빌드, subject, 표본 수, 발열, 비고, 파일을 라벨과 값 한 쌍씩 표시합니다. 그 아래에 눈금이 가리키는 기준 run(시각·빌드 이름·OS 빌드·발열)과 두 run의 빌드 비교를 이어 적습니다. |
...
- 막대 옆 변화량은 절댓값과 변화율을 함께 적습니다(`+10 ms · +4%`). 개수 지표는 작은 개수의 백분율이 오해를 부르므로 절댓값만 적습니다.
