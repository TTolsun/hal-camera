...
1. `두 실행 비교`를 누르고 기준과 현재 실행을 차례로 고릅니다. 선택한 기준은 밝은 파란 테두리로 표시하며 취소나 뒤로 가기로 선택을 해제합니다. 비교 화면은 결과 화면의 `비교`와 같은 변화율 막대 그래프 카드를 씁니다. 같은 두 실행 비교가 들어온 곳에 따라 그래프와 글자 행으로 달리 보이면 어색하기 때문입니다. 기준이 현재 baseline이 아니면 판정하지 않으므로 막대는 회색으로 방향과 크기만 보여 줍니다.
