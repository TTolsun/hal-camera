...
- 변화량은 막대 아래 줄의 오른쪽에 절댓값과 변화율을 함께 적습니다(`+10 ms · +4%`). 윗줄에는 지표 이름과 값만 둡니다. 변화율을 붙이자 한 줄에 세 가지가 들어가지 않아 이름과 값이 붙어 보였기 때문입니다. 개수 지표는 작은 개수의 백분율이 오해를 부르므로 절댓값만 적습니다.
- 기준 run에만 있는 지표도 행을 남깁니다. 이 경우 값은 `—`이고 눈금만 그리며, 막대 아래에 `이번 run에서 측정되지 않음`이라고 적습니다.
- 판정하지 못한 이유(`조건 불일치`, `표본 부족`, `단위 다름` 등)를 막대 아래 줄의 왼쪽에 적습니다.
