...
`두 실행 비교`와 `목록 CSV 내보내기`는 결과 화면의 `비교`·`내보내기`처럼 한 줄에 나란히 둡니다. 전체 폭 버튼 두 개와 개수 줄이 화면 위쪽 절반을 차지해 첫 실행이 화면 중간에서야 보였기 때문입니다. `목록 CSV 내보내기`는 아래 목록에 보이는 실행을 모두 CSV 파일 하나로 내보냅니다. 버튼 이름의 `목록`은 화면이나 실행 하나가 아니라 목록 전체가 파일에 들어간다는 뜻이며, 몇 개인지는 목록 제목과 화면 낭독기 설명에 있습니다.
...
1. `두 실행 비교`를 누르고 기준과 현재 실행을 차례로 고릅니다. 선택한 기준은 밝은 파란 테두리로 표시하며 취소나 뒤로 가기로 선택을 해제합니다. 비교 화면은 결과 화면의 `비교`와 같은 변화율 막대 그래프 카드를 씁니다. 같은 두 실행 비교가 들어온 곳에 따라 그래프와 글자 행으로 달리 보이면 어색하기 때문입니다. 기준이 현재 baseline이 아니면 판정하지 않습니다. 그래서 막대는 모두 회색으로 방향과 크기만 보여 줍니다. 비교 막대와 변화량 숫자의 색은 판정만 뜻합니다. 빨강은 저하, 초록은 개선, 회색은 판정하지 않은 변화입니다. 전에는 개선과 판정 없는 변화가 모두 파란색이라서, AE +78% 같은 참고용 변화도 좋은 소식처럼 보였고 누를 수 있는 요소처럼도 보였습니다.
