...
- 근거 수준: 코드 확인
- 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked)
...
| `camera/` | 엔진 계약, Camera2·CameraX 구현, 엔드포인트 열거를 제공합니다. `close(done)` 완료 전에 다음 카메라를 열지 않습니다. 사진·녹화 같은 미디어 동작은 `MediaCapture` 인터페이스가 계약이며, Live 화면은 구체 엔진 타입 대신 이 인터페이스로 확인합니다. 벤치마크 RECORD 단계의 recorder 상태 기계는 `BenchmarkRecorder`가 담당하고 `Camera2Engine`은 그 호출을 위임합니다. 카메라를 화면에 적는 이름(`Camera · 0 (Wide · Rear)`)도 이 패키지의 `CameraLabel` 하나가 만들며, Live·Benchmark·Probe·CTS가 모두 그것을 부릅니다. 렌즈 이름을 붙이지 못한 카메라에는 HAL이 보고한 35mm 환산 초점거리를 덧붙여 `Camera · 1 (Front · 26 mm)`처럼 적으므로, 전면 카메라가 둘인 기기에서도 목록이 두 항목을 구별합니다. |
...
| 항목 | 최신성 | 검토 |
| --- | --- | --- |
| 구조 원본 `data-flow` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
| 구조 원본 `overall-architecture` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
| 구조 원본 `state-transitions` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
| 구조 원본 `ui-camera-label` | 최신 | 검토 2026-09-25 @ `fffbc89` · K.H. Kim |
| 구조 원본 `ui-tool-handoff` | 최신 | 검토 2026-09-25 @ `fffbc89` · Claude (split MainActivity, code-checked) |
| 구조 원본 `ui-zoom` | 최신 | 검토 2026-09-25 @ `fffbc89` · K.H. Kim |
| 원고 `overview` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
| 원고 `module-roles` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
| 원고 `runtime-flow` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
| 원고 `constraints` | 최신 | 검토 2026-09-26 @ `9bf78dc` · Claude (merge history action row fix into compare chart, code-checked) |
