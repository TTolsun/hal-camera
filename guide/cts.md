...
CTS 원문 케이스의 판정은 메서드 하나에 하나입니다. `PASS`는 JUnit이 실패 없이 끝나고 카메라를 하나 이상 연 것, `FAIL`은 실패가 한 건 이상 있는 것, `SKIP`은 테스트가 assumption으로 스스로 건너뛰었거나 모든 카메라를 `continue`로 건너뛰어 카메라를 하나도 열지 않은 것입니다. 원문은 후자를 통과로 세지만, 이 앱은 가져온 `openDevice`에 심은 기록이 비어 있으면 SKIP으로 바꾸고, 테스트가 `Log`로 남긴 "… skipping" 문구를 프로세스 자신의 logcat에서 읽어 `건너뛴 이유` 카드에 보여 줍니다(예: `Camera 0 does not support HEIC_ULTRAHDR, skipping`). 앱은 자기 프로세스의 로그만 읽을 수 있으므로 카메라별·프로파일별 세부는 이 문구 이상으로 남지 않고, 실패 카드에는 예외 메시지와 CTS 코드 안의 호출 위치 세 줄만 남습니다. 어느 카메라에서 무엇이 실패했는지는 그 메시지에 적힌 카메라 ID로 읽습니다.
