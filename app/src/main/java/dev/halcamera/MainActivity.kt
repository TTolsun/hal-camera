...
import dev.halcamera.diagnosis.Assessment
import dev.halcamera.diagnosis.HealthLevel
import dev.halcamera.diagnosis.HealthMonitor
...
class MainActivity : ComponentActivity() {
    ...
    private fun buildUi() {
        ...
        val checkButton=button("검사") {
            ...
            startActivity(android.content.Intent(this,dev.halcamera.check.CheckActivity::class.java))
        }.apply { background=rounded(glass); setTextColor(Color.WHITE) }
        ...
        head.addView(LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; addView(label("HAL CAMERA",22,Color.WHITE,true)); addView(label("EXPERT DIAGNOSTICS  /  0.2",10,muted)) },LinearLayout.LayoutParams(0,-2,1f))
        ...
        tools.addView(button("기준 초기화") {
            AlertDialog.Builder(this).setTitle("검사 기준 초기화").setMessage("저장된 기준(baseline)을 모두 지웁니다. 다음 60초 검사가 새 기준이 됩니다.")
                .setNegativeButton("취소",null).setPositiveButton("초기화") { _,_ -> io.execute { dev.halcamera.baseline.BaselineStore(this).clear(); main.post { if(!destroyed) toast("기준을 지웠습니다") } } }.show()
    ...
    private fun updateHealth(events:List<Event>,frames:List<Event>,time:Long) {
        ...
        when(a.level) {
            ...
            HealthLevel.WATCH -> { healthBanner.background=rounded(dev.halcamera.ui.Look.statusWarn); healthBanner.setTextColor(Color.WHITE) }
    ...
    private fun consumerLines(a:Assessment?):Triple<String,String,List<String>> {
        ...
        val l1=dev.halcamera.diagnosis.DiagnosisRules.CONSUMER_TEXT[rule] ?: rule
        val l2=buildList {
