...
import dev.halcamera.check.AutoCheckRunner.Signal
import dev.halcamera.check.AutoCheckRunner.Step
...
class AutoCheckRunnerTest {
    ...
    @Test fun happyPathTwoEndpointsProducesLatencies() {
        ...
        assertEquals(dev.halcamera.diagnosis.UnknownReason.INSUFFICIENT_SAMPLES, samples["2.5"]!!.unknownReason)
    }
    ...
    @Test fun openTimeoutIsHardFailureAndNextEndpointStillRuns() {
        ...
        assertEquals(dev.halcamera.diagnosis.UnknownReason.NOT_RUN, r.log.done[0].launchSamples().first { it.id == "1.2" }.unknownReason)
        // After a failed open, close latency and shot-to-shot are not measurements of anything: NOT_RUN, not a value.
        assertEquals(dev.halcamera.diagnosis.UnknownReason.NOT_RUN, r.log.done[0].launchSamples().first { it.id == "1.7" }.unknownReason)
        assertEquals(dev.halcamera.diagnosis.UnknownReason.NOT_RUN, r.log.done[0].launchSamples().first { it.id == "2.5" }.unknownReason)
