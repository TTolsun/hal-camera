...
HAL CAM is a single-module Android application (`:app`, package `dev.halcamera`). Source lives in `app/src/main/java/dev/halcamera/`: `camera/` owns Camera2/CameraX engines and endpoint discovery; `telemetry/` records events; `metrics/` computes measurements; `benchmark/` runs profiles and comparisons, with pure Kotlin logic in `benchmark/domain/`, file and device adapters in `benchmark/platform/`, and only Activities at its root; `cli/` accepts ADB commands and hosts the screen adapters; `cts/` runs AOSP CTS camera test methods in-app (the sources live in the `:ctsvendor` module; no transcribed cases); `ui/` contains shared views and styling. Activities construct layouts in Kotlin. `benchmark/domain/` and `metrics/` must not import `android.*` or `org.json`, and `domain/` must not import `platform/`; `LayerIsolationTest` reads the sources and fails on either. Resources belong in `app/src/main/res/`, and JVM tests mirror packages under `app/src/test/java/dev/halcamera/`.
